...
router.post("/", async (req: Request, res: Response): Promise<void> => {
  ...
  const stream = await client.chat.completions.create({
    model: MODEL,
    stream: true,
    // deepseek-v4-pro 默认开思考，显式声明以示意图；思考链走 delta.reasoning_content。
  ...
  try {
    if (streamCompleted && fullText.trim()) {
      ...
      const nextMessages = [...messages, assistantMsg];
      ...
      if (convo) {
        ...
        const firstUser = messages.find(
          (m: ChatMsg) => m.role === "user",
        );
        const generated = firstUser
          ? await generateTitle(firstUser.content, fullText)
          : "";
        historyTitle = generated || makeTitle(messages);
      }
