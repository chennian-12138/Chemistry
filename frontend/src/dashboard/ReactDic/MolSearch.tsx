  ...
  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col gap-6 py-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="text-center space-y-2 mb-2">
        <h2 className="text-2xl font-semibold tracking-tight">{t("structure.title")}</h2>
        <p className="text-muted-foreground text-sm">{t("structure.hint")}</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-2">
        <div className="flex items-center gap-2 text-sm text-muted-foreground min-h-[1.5rem]">
          {molCount > 0 ? (
            <span>
              {molCount === 1 ? t("structure.detectOne") : (
                <>
                  <span className="font-semibold text-foreground">{molCount}</span>{" "}
                  {t("structure.detectMultiple")}
                </>
              )}
            </span>
          ) : (
            <span>{t("structure.drawHint")}</span>
          )}
        </div>
        ...
        >
          {isLoading ? t("search.searching") : t("structure.button")}
        </Button>
      </div>
    </div>
  );
