...
const FEATURES: {
  ...
  { type: "AI_CHAT", labelKey: "stats.aiChat", unitKey: "stats.sessionUnit", icon: Bot, light: "#27374D", dark: "#C0E1D2" },
  { type: "RETRO_SYNTHESIS", labelKey: "stats.retro", unitKey: "stats.routeUnit", icon: FlaskConical, light: "#526D82", dark: "#E5EEE4" },
  { type: "REACTDIC", labelKey: "stats.reactdic", unitKey: "stats.reactionUnit", icon: BookSearch, light: "#9DB2BF", dark: "#F6F4E8" },
  { type: "PAPER", labelKey: "stats.paper", unitKey: "stats.paperUnit", icon: Newspaper, light: "#DDE6ED", dark: "#DC9B9B" },
];

// shadcn ChartConfig：每个功能一条 series，theme 提供明/暗两套色
// chartConfig 只保存颜色；展示名称由组件内 t(f.labelKey) 提供
const chartConfig = FEATURES.reduce((acc, f) => {
  acc[f.type] = { label: f.labelKey, theme: { light: f.light, dark: f.dark } };
  return acc;
}, {} as ChartConfig);
  ...
  return (
    <div className="flex flex-col gap-6">
            ...
            <Card key={f.type}>
              <CardContent className="pt-6">
                <div className="flex items-center gap-2 text-muted-foreground">
                  ...
                  <span className="text-sm font-medium truncate">{t(f.labelKey as any)}</span>
                </div>
      ...
      <Card className="flex flex-col">
        <CardHeader>
          <CardTitle className="text-base">{t("stats.trendTitle")}</CardTitle>
          <CardDescription className="text-sm">{t("stats.trendDesc")}</CardDescription>
        </CardHeader>
